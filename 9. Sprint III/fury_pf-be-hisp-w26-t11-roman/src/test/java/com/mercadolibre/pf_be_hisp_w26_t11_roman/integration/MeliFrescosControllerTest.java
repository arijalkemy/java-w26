package com.mercadolibre.pf_be_hisp_w26_t11_roman.integration;
...
import com.mercadolibre.pf_be_hisp_w26_t11_roman.repository.IInboundOrderRepository;
import com.mercadolibre.pf_be_hisp_w26_t11_roman.repository.IUserRepository;
...
import com.mercadolibre.pf_be_hisp_w26_t11_roman.dto.frescos.PurchaseOrderDTO;
import com.mercadolibre.pf_be_hisp_w26_t11_roman.dto.frescos.PurchaseRequestDTO;
import com.mercadolibre.pf_be_hisp_w26_t11_roman.dto.frescos.PurchasedProductDTO;
import com.mercadolibre.pf_be_hisp_w26_t11_roman.dto.frescos.StatusDTO;
import com.mercadolibre.pf_be_hisp_w26_t11_roman.repository.IProductRepository;
import com.mercadolibre.pf_be_hisp_w26_t11_roman.repository.ISectionRepository;
import com.mercadolibre.pf_be_hisp_w26_t11_roman.repository.IWarehouseRepository;
...
import com.mercadolibre.pf_be_hisp_w26_t11_roman.service.AuthServiceImpl;
...
//                .andExpect(jsonPath("$.batch_stock_extended_dtolist[1].due_date").value(currentDate.plusDays(7).toString()));
    ...
    @Test
    @DisplayName("Testeando el registro de una orden de compra 200 con un batch")
    void registerOrder() throws Exception {
        ...
        PurchaseOrderDTO purchaseOrder = new PurchaseOrderDTO(LocalDate.of(2024, 6, 1), 5,
    ...
    void updateOrder() throws Exception {
        ...
        PurchaseOrderDTO purchaseOrder = new PurchaseOrderDTO(LocalDate.of(2024, 6, 1), 3,
                new StatusDTO("Carrito"),
    ...
    public void createInboundOrderTest() throws Exception {
        batchStockRequestDTO.setBatchNumber(6);
    ...
    void testListBatchesOfProductInPurchaseOrder() throws Exception {
        // Arrange
        Integer orderId = 2;
        AuthResponseDto authResponseDto = authService.login(new LoginRequestDto("rep_user1", "password1"));

        // Act & Assert
        mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/fresh-products/orders/{orderId}/batch-list", orderId)
                        .header("Authorization", "Bearer " + authResponseDto.getToken()))
                .andExpect(status().isOk())
    ...
    @Test
    @DisplayName("Requerimiento 6.1: Mostrar los batches de una orden para el proceso de Picking 404")
    void testListBatchesOfProductInPurchaseOrder404() throws Exception {
        // Arrange
        Integer orderId = 100;
        AuthResponseDto authResponseDto = authService.login(new LoginRequestDto("rep_user1", "password1"));
    ...
    @Test
    @DisplayName("Requerimiento 6.2: Mostrar los productos que estan en un carrito de compras")
    void testListProductsInCart() throws Exception {
        // Arrange
        AuthResponseDto authResponseDto = authService.login(new LoginRequestDto("rep_user1", "password1"));

        // Act & Assert
        String result = mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/fresh-products/products/cart")
                        .header("Authorization", "Bearer " + authResponseDto.getToken()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray())
                .andExpect(jsonPath("$").isNotEmpty())
        ...
        int savedQuantity1 = JsonPath.parse(result).read("$[0].saved_quantity");
        int savedQuantity2 = JsonPath.parse(result).read("$[1].saved_quantity");

        Assertions.assertTrue(savedQuantity1 > savedQuantity2);
    }
