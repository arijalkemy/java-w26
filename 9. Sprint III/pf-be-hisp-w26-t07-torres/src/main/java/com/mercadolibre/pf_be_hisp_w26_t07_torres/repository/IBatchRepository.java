...
public interface IBatchRepository extends JpaRepository<Batch, Long> {
    ...
    @Query("""
            SELECT b.id as batchId, b.productSeller.id as productId, b.currentTemperature as temperature,
            b.minimumTemperature as minTemperature
            FROM Batch b WHERE ABS(b.currentTemperature - b.minimumTemperature) > :threshold
            """)
    List<IBatchTemperatures> findBatchesWithTemperatureDifference(@Param("threshold") double threshold);

    @Query("""
            SELECT b.id as batchId, b.productSeller.id as productId,
            b.currentQuantity as currentQuantity, b.initialQuantity as initialQuantity
            FROM Batch b WHERE b.currentQuantity < cast(b.initialQuantity as float) * :threshold
            """)
    List<IBatchLowStock> findBatchesWithLowStock(@Param("threshold") double threshold);
}
