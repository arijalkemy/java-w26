...
public class ProductController {
    ...
    public ResponseEntity<ProductCreateResponseDto> createProductSeller(
            @PathVariable Long idSeller,
            @RequestBody @Valid ListProductRequestDto productRequestDto
    ) {
        ProductCreateResponseDto productCreatedResponse = productService.createProductSeller(idSeller, productRequestDto);
        return ResponseEntity.status(HttpStatus.CREATED).body(productCreatedResponse);
    }

    @PutMapping("/{idSeller}/{idProduct}")
    @PreAuthorize("hasRole('ROLE_SELLER')")
    public ResponseEntity<ProductCreateResponseDto> updateProductSeller(
            @PathVariable Long idProduct, @PathVariable Long idSeller,
            @RequestBody @Valid ProductRequestDto productRequestDto
    ) {
        ProductCreateResponseDto responseDto = productService.updateProductSeller(idSeller, idProduct, productRequestDto);
