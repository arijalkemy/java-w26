...
public class InboundOrderServiceImpl implements IInboundOrderService{
    ...
    public CartWithErrorsResponseDTO createInBoundOrder(CartDTO cartDTO) {
        //Turn cartDTO into PurchaseDTO
        PurchaseOrderDTO purchaseOrderDTO = cartDTO.getPurchaseOrder();
        //Search if buyer_id exists
        Optional<User> optionalUser = userRepository.findById((long)cartDTO.getPurchaseOrder().getIdBuyer());
        if(!optionalUser.isPresent()){
            throw new ApiException("Not Found", "User doesn't exist", 404);
        }
        //Check if it's a user
        if(!optionalUser.get().getRole().equals(Rol.USER)){
            throw new ApiException("Not Found", "Id isn't a user", 404);
        }
        //We create a User
        User user = User.builder()
                .id((long)cartDTO.getPurchaseOrder().getIdBuyer())
                .build();
        //Get all the products
        List<ProductDTO> productDTOS = purchaseOrderDTO.getProductDTO();
        //List of errors
        List<ApiError> errors = new ArrayList<>();
        //List of products in cart
        List<ProductInCart> productsInCarts = new ArrayList<>();
        Double totalPrice = 0.0;
        //For every product, we search their stock in BatchStock
        for(ProductDTO productDTO:productDTOS) {
            Optional<Product> optionalProduct = productRepository.findById((long) productDTO.getIdProduct());
            if (!optionalProduct.isPresent()) {
                throw new ApiException("Not Found", "Product not found", 404);
            }
            Product product = optionalProduct.get();
            List<BatchStock> batchStocks = batchStockRepository.getBatchStocksByProductId((long) productDTO.getIdProduct());
            Integer totalStocks = batchStocks.stream().
                    mapToInt(BatchStock::getCurrentQuantity).
                    sum();
            ...
            //We create a productInCart, that we will be adding in each product
            Product auxProduct = Product.builder()
                    .id(product.getId())
                    .build();
        ...
        //We check if there is not enough stock
        if(errors.size() > 0)
        ...
        Cart cart = Cart.builder()
                ...
                .buyer(user)
                .build();
        //We save the cart in the repository
        cartRepository.save(cart);
        //We add each product in cart the id of the
        productsInCarts.stream()
                .peek(productInCart -> productInCart.setCart(cart))
                .forEach(productInCartRepository::save);
        //We return the response with the total
