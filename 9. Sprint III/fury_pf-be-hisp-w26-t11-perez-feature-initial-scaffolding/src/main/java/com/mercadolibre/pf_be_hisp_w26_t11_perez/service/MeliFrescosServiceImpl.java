...
import com.mercadolibre.pf_be_hisp_w26_t11_perez.exceptions.ApiException;
import com.mercadolibre.pf_be_hisp_w26_t11_perez.exceptions.BadRequestException;
import com.mercadolibre.pf_be_hisp_w26_t11_perez.exceptions.NotFoundException;
import com.mercadolibre.pf_be_hisp_w26_t11_perez.model.*;
import com.mercadolibre.pf_be_hisp_w26_t11_perez.repository.*;
import com.mercadolibre.pf_be_hisp_w26_t11_perez.util.ValidationUtils;
...
public class MeliFrescosServiceImpl implements IMeliFrescosService {
    ...
    @Override
    @Transactional
    public PurchaseOrderDetailDTO registerPurchaseOrder(Integer orderId) {
        PurchaseOrder purchaseOrder = purchaseOrderRepository.findById(orderId)
                .orElseThrow(() -> new NotFoundException("Purchase Order not found"));

        if(!purchaseOrder.getStatus().getStatusCode().equals("Carrito"))
            throw new ApiException("Orden no válida", "bad_request", 400);

        purchaseOrder.setDate(LocalDate.now());
        purchaseOrder.setStatus(statusRepository.findByStatusCode("Completado")
        ...
        List<PurchaseOrderProduct> purchaseOrderProduct = purchaseOrderProductRepository.findAllByPurchaseOrder(purchaseOrder);
        purchaseOrderProduct.forEach(purchaseProduct -> {
            productDetailDTOList.add(ProductDetailDTO.builder()
                    .name(purchaseProduct.getProduct().getName())
                    .price(purchaseProduct.getProduct().getPrice())
                    .quantity(purchaseProduct.getProductQuantity())
                    .build());

            PageRequest pageRequest = PageRequest.of(0, 10);
            List<Batch> batches = batchRepository.findNearestDueDate(LocalDate.now(), purchaseProduct.getProduct().getId(), pageRequest);

            int quantityToDiscount = purchaseProduct.getProductQuantity();
            for (Batch batch : batches) {
                if (batch.getCurrentQuantity() >= quantityToDiscount) {
                    batch.setCurrentQuantity(batch.getCurrentQuantity() - quantityToDiscount);
                    batchRepository.save(batch);
                    break;
                } else {
                    quantityToDiscount -= batch.getCurrentQuantity();
        ...
        return PurchaseOrderDetailDTO.builder()
                ...
                .orderStatus(purchaseOrder.getStatus().getStatusCode())
                .totalCost(productDetailDTOList.stream().mapToDouble(p -> p.getPrice() * p.getQuantity()).sum())
                .date(purchaseOrder.getDate().toString())
                .buyerId(purchaseOrder.getBuyer().getId())
                .products(productDetailDTOList)
                .build();
    }

    @Override
    public List<BatchStockResponseDTO> listAllExpiredBatches() {
        List<Batch> batches = batchRepository.findAllByDueDateBefore(LocalDate.now());
        return List.of(modelMapper.map(batches, BatchStockResponseDTO[].class));
    }
