...
public class ProductControllerTest {
    ...
    @Test
    @DisplayName("Given product details, when postCreateProduct is called, then it should return created response")
    @WithMockUser(roles = "SELLER")
    void testPostCreateProduct() throws Exception {
        // Create a test user
        User user = new User();
        user.setFirstName("testUser");
        user.setEmail("test56777@test.com");
        user.setPassword("password");
        user.setRole(Role.SELLER);
        userRepository.save(user);
        // Generate token with an additional claim "role"
        ...
        String requestBody = objectMapper.writeValueAsString(Collections.singletonList(productDetailDTO));
        // Perform POST request
        ResultActions result = mockMvc.perform(post("/api/v1/fresh-products/{idSeller}", 1L)
                .contentType(MediaType.APPLICATION_JSON)
                .content(requestBody)
                .header("Authorization", "Bearer " + token)
                .accept(MediaType.APPLICATION_JSON));
        ...
        result.andExpect(status().isCreated())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.message").value("The products has been created"))
                .andDo(print());
    }

    @Test
    @DisplayName("Given no product details, when postCreateProduct is called, then it should return bad request")
    @WithMockUser(roles = "SELLER")
    void testPostCreateProduct_Failure() throws Exception {
        // Create a test user
        User user = new User();
        user.setFirstName("testUser");
        user.setEmail("test23489@test.com");
        user.setPassword("password");
        user.setRole(Role.SELLER);
        userRepository.save(user);
        // Generate token with an additional claim "role"
    ...
    @Test
    @DisplayName("Given product details, when postCreateProduct is called, then it should return OK response")
    @WithMockUser(roles = "SELLER")
    void testPostCreateProduct_Success() throws Exception {
        // Create a test user
        User user = new User();
        user.setFirstName("testUser");
        user.setEmail("test4567@test.com");
        user.setPassword("password");
        user.setRole(Role.SELLER);
        userRepository.save(user);
        // Generate token with an additional claim "role"
        ...
        productDetailDTO.setDescription("Test Product");
        ObjectMapper objectMapper = new ObjectMapper();
        String requestBody = objectMapper.writeValueAsString(productDetailDTO);
        // Perform PUT request
        ResultActions result = mockMvc.perform(put("/api/v1/fresh-products/{idSeller}/{idProduct}", 1L, 1L)
                .contentType(MediaType.APPLICATION_JSON)
                .content(requestBody)
                .header("Authorization", "Bearer " + token)
                .accept(MediaType.APPLICATION_JSON));
        ...
        result.andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.message").value("The product has been updated"));
    }

    @Test
    @DisplayName("Given no product details, when postCreateProduct is called, then it should return bad request")
    @WithMockUser(roles = "SELLER")
    void testPutUpdateProduct_Failure() throws Exception {
        // Create a test user
        User user = new User();
        user.setFirstName("testUser");
        user.setEmail("test56789@test.com");
        user.setPassword("password");
        user.setRole(Role.SELLER);
        userRepository.save(user);
        // Generate token with an additional claim "role"
        ...
        extraClaims.put("role", "SELLER");
        String token = jwtTokenService.generateToken(extraClaims, user);
        // Perform PUT request without request body
        ResultActions result = mockMvc.perform(put("/api/v1/fresh-products/{idSeller}/{idProduct}", 1L, 1L)
                .contentType(MediaType.APPLICATION_JSON)
                .header("Authorization", "Bearer " + token)
                .accept(MediaType.APPLICATION_JSON));
