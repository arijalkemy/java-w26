...
import com.mercadolibre.pf_be_hisp_w26_t7_villarraga.mapper.BachMapper;
import com.mercadolibre.pf_be_hisp_w26_t7_villarraga.mapper.StockMapper;
...
public class InboundorderServiceImpl implements IInboundorderService {
    ...
    @Override
    @Transactional
    public void deleteBatchByNumberAndProduct(Long batchNumber, Long productId) {
        Long representativeId = authDataUtil.getIdSession();
        Batch batch = getBatchByNumber(batchNumber, productId);
        Representative representative = getRepresentativeById(representativeId);
        if (batch == null) {
            throw new NotFoundException(MessageError.BATCH_NOT_FOUND_NUMBER.getMessage(batchNumber));
        }
        if (!batch.getRepresentative().equals(representative)) {
            throw new NotFoundException(MessageError.REPRESENTATIVE_NOT_ASSOCIATED_WITH_WAREHOUSE.getMessage());
        }
