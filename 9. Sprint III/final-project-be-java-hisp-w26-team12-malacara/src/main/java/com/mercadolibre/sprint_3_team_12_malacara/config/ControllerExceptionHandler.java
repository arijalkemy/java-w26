package com.mercadolibre.sprint_3_team_12_malacara.config;
...
public class ControllerExceptionHandler {
  ...
  /**
   * Handler for bad credentials exceptions.
   * @param e the exception thrown when credentials are invalid.
   * @return {@link ResponseEntity} with 401 status code and description indicating bad credentials.
   */
  @ExceptionHandler(BadCredentialsException.class)
  protected ResponseEntity<ApiError> handleBadCredentialsException(BadCredentialsException e) {
    ApiError apiError =
            new ApiError(
                    "Bad credentials", "Invalid username or password", HttpStatus.UNAUTHORIZED.value());
    return ResponseEntity.status(apiError.getStatus()).body(apiError);
  }
}
