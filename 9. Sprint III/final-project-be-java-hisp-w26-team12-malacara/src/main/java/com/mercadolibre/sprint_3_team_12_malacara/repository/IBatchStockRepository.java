...
import com.mercadolibre.sprint_3_team_12_malacara.entity.BatchStock;
...
public interface IBatchStockRepository extends JpaRepository<BatchStock,Long> {
    ...
    @Query("SELECT new com.mercadolibre.sprint_3_team_12_malacara.dto.response.BatchDueDTO (bs.id, bs.product.id, p.type, bs.dueDate, bs.currentQuantity)" +
            "FROM BatchStock bs " +
    ...
    @Query("SELECT NEW  com.mercadolibre.sprint_3_team_12_malacara.projections.BatchStockProjection(bs.id, bs.currentQuantity, bs.dueDate) " +
            "FROM BatchStock bs " +
    ...
    @Query("SELECT NEW  com.mercadolibre.sprint_3_team_12_malacara.projections.WarehouseProjection(s.warehouse.id, SUM(bs.currentQuantity)) " +
            "FROM BatchStock bs " +
            "JOIN bs.inboundOrder io " +
            "JOIN io.section s " +
            "WHERE bs.product.id = :productId " +
    ...
    @Query("SELECT new com.mercadolibre.sprint_3_team_12_malacara.dto.response.BatchDueDTO(bs.id, bs.product.id, p.type, " +
