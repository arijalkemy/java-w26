...
import com.mercadolibre.pf_be_hisp_w26_t11_acontrerasc.dto.frescos.ProductBatchesResponseDTO;
...
import com.mercadolibre.pf_be_hisp_w26_t11_acontrerasc.service.IMeliFrescosService;
...
public class MeliFrescosController {
    ...
    @Autowired
    private MeliFrescosServiceImpl meliFrescosServiceImpl;
     ...
     * @throws com.mercadolibre.meli_frescos.exceptions.NotFoundException
     */
    ...
    /*
        ...
        ▂▃▄▅▆▇█▓▒░REQUERIMENTOS INDIVIDUALES░▒▓█▇▆▅▄▃▂



     */

    @PostMapping("/fresh-products/add")
    public ResponseEntity<ProductStatusDto> registerProduct( @RequestBody @Valid ProductRequestDto productRequestDto)
    {
        return new ResponseEntity<>(meliFrescosServiceImpl.createProduct(productRequestDto), HttpStatus.CREATED);
    }

    @PutMapping("/fresh-products/update")
    public ResponseEntity<ProductStatusDto> updateProduct( @RequestBody @Valid ProductRequestDto productResponseDto)
    {
        return new ResponseEntity<>(meliFrescosServiceImpl.updateProduct(productResponseDto), HttpStatus.OK);
    }

    @PostMapping("/fresh-products/add/bulk")
    public ResponseEntity<ProductStatusDto> addProductsBulk( @RequestBody @Valid List<ProductRequestDto> productRequestDto)
    {
        return new ResponseEntity<>(meliFrescosServiceImpl.addProductsBulk(productRequestDto), HttpStatus.CREATED);
    }

    @GetMapping("/fresh-products/list/batches")
    public ResponseEntity<List<ProductResponseWithTypeDTO>> getProductsById() {
        return new ResponseEntity<>(meliFrescosServiceImpl.getProductsBatches(), HttpStatus.OK);
    }
}
