package com.mercadolibre.pf_be_hisp_w26_t11_acontrerasc.integration;
...
import com.mercadolibre.pf_be_hisp_w26_t11_acontrerasc.repository.IInboundOrderRepository;
import com.mercadolibre.pf_be_hisp_w26_t11_acontrerasc.repository.IUserRepository;
...
import com.mercadolibre.pf_be_hisp_w26_t11_acontrerasc.dto.frescos.PurchaseOrderDTO;
import com.mercadolibre.pf_be_hisp_w26_t11_acontrerasc.dto.frescos.PurchaseRequestDTO;
import com.mercadolibre.pf_be_hisp_w26_t11_acontrerasc.dto.frescos.PurchasedProductDTO;
import com.mercadolibre.pf_be_hisp_w26_t11_acontrerasc.dto.frescos.StatusDTO;
import com.mercadolibre.pf_be_hisp_w26_t11_acontrerasc.repository.IProductRepository;
import com.mercadolibre.pf_be_hisp_w26_t11_acontrerasc.repository.ISectionRepository;
import com.mercadolibre.pf_be_hisp_w26_t11_acontrerasc.repository.IWarehouseRepository;
...
//                .andExpect(jsonPath("$.batch_stock_extended_dtolist[1].due_date").value(currentDate.plusDays(7).toString()));
    ...
    /*
        ...
        ▂▃▄▅▆▇█▓▒░REQUERIMENTOS INDIVIDUALES TESTING░▒▓█▇▆▅▄▃▂
    ...
    public void registerProductTest() throws Exception {
        ProductRequestDto product = new ProductRequestDto("Apples", 30.00);
        AuthResponseDto authResponseDto = authService.login(new LoginRequestDto("seller_user1", "password1"));
        mockMvc.perform(post("/api/v1/fresh-products/add")
                        .header("Authorization", "Bearer " + authResponseDto.getToken())
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    public void registerProductTestWithAUnValidUser() throws Exception {
        ProductRequestDto product = new ProductRequestDto("Apples", 30.00);
        AuthResponseDto authResponseDto = authService.login(new LoginRequestDto("buyer_user1", "password1"));
        mockMvc.perform(post("/api/v1/fresh-products/add")
                        .header("Authorization", "Bearer " + authResponseDto.getToken())
    ...
    @Test
    public void verifyBadRequestWhenAProductRequestDtoIsSendedWithOutId() throws Exception
    {
        ProductRequestDto product = new ProductRequestDto("Apples", 30.00);
        AuthResponseDto authResponseDto = authService.login(new LoginRequestDto("seller_user1", "password1"));
        mockMvc.perform(put("/api/v1/fresh-products/update")
                        .header("Authorization", "Bearer " + authResponseDto.getToken())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(product)))
                .andExpect(status().isBadRequest());
    ...
    @Test
    public void verifyThe404NotFoundOnUpdateProduct() throws Exception
    {
        ProductRequestDto product = new ProductRequestDto(101, "Apples", 30.00);
        AuthResponseDto authResponseDto = authService.login(new LoginRequestDto("seller_user1", "password1"));
        mockMvc.perform(put("/api/v1/fresh-products/update")
                        .header("Authorization", "Bearer " + authResponseDto.getToken())
                        .contentType(MediaType.APPLICATION_JSON)
