package com.mercadolibre.project_be_java_hisp_w26_team4.integration;
...
class FreshProductControllerTest {
    ...
    void postOrderWithMissingProduct() throws Exception {
        Long idOrder = 1L;
        String token = jwtService.getToken(User.builder()
                .username("buyer")
                .password("buyer")
    ...
    @Test
    @DisplayName("getProductLocationsByType with prodId 1 returns 200 OK")
    void getProductLocationsByType_withProdId1_returns200OK() throws Exception {
        Long prodId = 1L;
        String token = jwtService.getToken(User.builder()
                .username("manager")
                .password("manager")
        ...
        mockMvc.perform(get("/api/v1/fresh_products/{prodId}/batch/list", prodId)
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk());
    }
