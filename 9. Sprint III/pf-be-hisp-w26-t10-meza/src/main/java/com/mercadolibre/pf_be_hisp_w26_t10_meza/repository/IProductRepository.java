...
public interface IProductRepository extends JpaRepository<Product, Integer> {
     ...
     */
    @Query("select new com.mercadolibre.pf_be_hisp_w26_t10_meza.dtos.InventoryByWarehouseDto (w.id, sum(b.initialQuantity)) from Product p inner join Batch b on p.id = b.product.id inner join Sector s on b.sector.id = s.id inner join Warehouse w on s.warehouse.id = w.id where p.id = :product_id group by w.id")
