...
public class ProductServiceTest {
    ...
    /**
     *Test to see a new register of a list of products
     */
    @Test
    @DisplayName("Test of adding a product")
    void testCreateProduct(){
        //Arrange
        MessageDTO expectedMessage = new MessageDTO("Products added to the database");
        //Act
        List<ProductAddDTO> productAddDTOS = List.of(
                new ProductAddDTO("Product 1", "Product", 10.9, Category.FF),
                new ProductAddDTO("Product 2", "Product", 11.9, Category.FS)
        );
        ProductInjectionDTO productInjectionDTO = new ProductInjectionDTO();
        productInjectionDTO.setProductAddDTOS(productAddDTOS);
        MessageDTO actualMessage = productService.postProducts(productInjectionDTO);
        //Assert
        Assertions.assertEquals(expectedMessage, actualMessage);
    }

    /**
     * Test to see the updated product
     */
    @Test
    @DisplayName("Test of updating a product")
    void testUpdateProduct(){
        //Arrange
        MessageDTO expectedMessage = new MessageDTO("Product updated on the database");
        Product product = Product.builder()
                ...
                .price(100.99)
                .build();
        ProductAddDTO productAddDTO = new ProductAddDTO("Product 1", "Product", 10.9, Category.FF);
        //Act
        when(productRepository.findByName("Product 1")).thenReturn(Optional.ofNullable(product));
        MessageDTO actualMessage = productService.updateProduct(productAddDTO);
        //Assert
        Assertions.assertEquals(expectedMessage, actualMessage);
    }
}
