...
import com.mercadolibre.pf_be_hisp_w26_t10_imbachi.service.implementations.ShoppingCartServiceImpl;
...
public class FreshProductsController {
    ...
    @GetMapping("batch/list/low-quantity/{quantity}")
    public ResponseEntity<?> getBatchesWithLowQuantities(@PathVariable Integer quantity){
        return ResponseEntity.status(HttpStatus.OK).body(batchService.getLowQuantityBatch(quantity));
    }
    ...
    @GetMapping("/warehousesupervisor/{supervisor_id}")
    public ResponseEntity<?> getWarehouseSupervisor(@PathVariable Long supervisor_id){
        return ResponseEntity.status(HttpStatus.OK).body(warehouseService.getWhBySupervisor(supervisor_id));
    }
}
