package com.mercadolibre.pf_be_hisp_w26_t11_perez.integration;
...
//    }
    ...
    @Test
    @DisplayName("Cambiar el estado de una orden de compra a 'Completo'")
    public void testChangeOrderStatusToComplete() throws Exception {
        // Arrange
        Integer orderId = 1;

        // Act & Assert
        AuthResponseDto authResponseDto = authService.login(new LoginRequestDto("buyer_user1", "password1"));
        this.mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/fresh-products/ordersById/" + orderId)
                        .header("Authorization", "Bearer " + authResponseDto.getToken())
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk())
    ...
    @Test
    @DisplayName("Cambiar el estado de una orden de compra a 'Completo' con un id de orden no existente")
    public void testChangeOrderStatusToComplete_OrderNotFound() throws Exception {
        // Arrange
    ...
    @Test
    @DisplayName("Listar los lotes de productos vencidos")
    public void testListExpiredBatches() throws Exception {
        // Arrange
        // Act & Assert
        AuthResponseDto authResponseDto = authService.login(new LoginRequestDto("rep_user1", "password1"));
        this.mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/expired-batches")
                        .header("Authorization", "Bearer " + authResponseDto.getToken())
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk())
