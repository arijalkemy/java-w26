...
import com.mercadolibre.pf_be_hisp_w26_t11_perez.service.IMeliFrescosService;
...
public class MeliFrescosController {
     ...
     * @throws com.mercadolibre.pf_be_hisp_w26_t11_perez.exceptions.NotFoundException
     */
    ...
    @GetMapping("/fresh-products/ordersById/{id}")
        public ResponseEntity<PurchaseOrderDetailDTO> registerPurchaseOrder(@PathVariable Integer id) {
        return new ResponseEntity<>(service.registerPurchaseOrder(id), HttpStatus.OK);
    }

    @GetMapping("/expired-batches")
    public ResponseEntity<List<BatchStockResponseDTO>> listPurchaseOrders() {
        return new ResponseEntity<>(service.listAllExpiredBatches(), HttpStatus.OK);
    }
