package com.mercadolibre.pf_be_hisp_w26_t10_garcia.unit.get_cart_products;
...
public class FrescosServiceTest {
    ...
    public void getShoppingCartProducts(){
        ...
        cartProduct.setId(1);
        cartProduct.setProduct(new Product(1,null,"Pizza de Pepperoni",8.99,null,null,null));
        products.add(cartProduct);
        List<ProductDto> productDtos = new ArrayList<>();
        productDtos.add(new ProductDto(
                cartProduct.getProduct().getName(),
                cartProduct.getProduct().getPrice()));
        when(shoppingCartProductRepository.findDistinctByShoppingCardId(idOrder)).thenReturn(products);

        // Act
        List<ProductDto> response = frescosService.getProductsFromShoppingCart(idOrder).stream().toList();

        // Assert
        verify(shoppingCartProductRepository, atLeastOnce()).findDistinctByShoppingCardId(idOrder);
