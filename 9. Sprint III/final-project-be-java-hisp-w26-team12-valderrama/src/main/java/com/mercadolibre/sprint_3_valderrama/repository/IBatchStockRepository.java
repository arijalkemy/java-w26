...
import com.mercadolibre.sprint_3_valderrama.entity.BatchStock;
...
public interface IBatchStockRepository extends JpaRepository<BatchStock,Long> {
    ...
    @Query("SELECT new com.mercadolibre.sprint_3_valderrama.dto.response.BatchDueDTO (bs.id, bs.product.id, p.type, bs.dueDate, bs.currentQuantity)" +
            "FROM BatchStock bs " +
    ...
    @Query("SELECT new com.mercadolibre.sprint_3_valderrama.dto.response.BatchDueDTO (bs.id, bs.product.id, p.type, bs.dueDate, bs.currentQuantity)" +
            ...
            "INNER JOIN ibo.section s " +
            "WHERE s.warehouse.id = :warehouseId " +
            "AND bs.currentQuantity = 0")
    List<BatchDueDTO> findBatchStockByCurrentQuantityOut(Long warehouseId);

    List<BatchStock> getBatchStocksByProductId(Long id);

    List<BatchStockProjection>  findByProductId(Long productID);

    @Query("SELECT NEW  com.mercadolibre.sprint_3_valderrama.projections.BatchStockProjection(bs.id, bs.currentQuantity, bs.dueDate) " +
            "FROM BatchStock bs " +
    ...
    @Query("SELECT NEW  com.mercadolibre.sprint_3_valderrama.projections.WarehouseProjection(s.warehouse.id, SUM(bs.currentQuantity)) " +
            "FROM BatchStock bs " +
            "JOIN bs.inboundOrder io " +
            "JOIN io.section s " +
            "WHERE bs.product.id = :productId " +
    ...
    @Query("SELECT new com.mercadolibre.sprint_3_valderrama.dto.response.BatchDueDTO(bs.id, bs.product.id, p.type, " +
