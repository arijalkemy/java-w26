...
import com.mercadolibre.sprint_3_valderrama.dto.response.ResponseBatchDTO;
import com.mercadolibre.sprint_3_valderrama.dto.response.WarehouseDTO;
...
import com.mercadolibre.sprint_3_valderrama.projections.BatchStockProjection;
import com.mercadolibre.sprint_3_valderrama.projections.SectionProjection;
import com.mercadolibre.sprint_3_valderrama.projections.WarehouseProjection;
...
public class BatchService implements IBatchService {
     ...
     * @throws com.mercadolibre.sprint_3_valderrama.exceptions.ApiException - If an exception occurs while retrieving the
    ...
    @Override
    public ResponseBatchDueDTO getExpiredBatches(Long warehouseId) {
        List<BatchDueDTO> list = batchStockRepository
                .findProductsBetweenDate(warehouseId,Date.valueOf(LocalDate.of(1, 1, 1)), Date.valueOf(LocalDate.now()));
        if (list.isEmpty()) {
            throw new ApiException("Not Found", "Not have expired Products in this warehouse", HttpStatus.NOT_FOUND.value());
        }
        return new ResponseBatchDueDTO(list);
    }

    @Override
    public ResponseBatchDueDTO getOutedBatches(Long warehouseId) {
        List<BatchDueDTO> list = batchStockRepository
                .findBatchStockByCurrentQuantityOut(warehouseId);
        if (list.isEmpty()) {
            throw new ApiException("Not Found", "Haven´t batches without stock", HttpStatus.NOT_FOUND.value());
        }
        return new ResponseBatchDueDTO(list);
    }
