...
import com.mercadolibre.sprint3_individual_perez.dto.request.*;
...
import com.mercadolibre.sprint3_individual_perez.dto.response.*;
...
public class InboundOrderServiceImpl implements IInboundOrderService{
    ...
    /**
     * Method to get the inbound orders of the admin
     * @return the list of inbound orders
     */
    @Override
    public InboundListResponseDTO getInboundOrders() {
        User user = getUserFromAuthentication();
        Optional<List<InboundOrder>> inboundOrders = inboundOrderRepository.getInboundOrdersByUserId(user.getId());
        if (inboundOrders.isPresent()) {

            return new InboundListResponseDTO().builder()
                    .inboundResponseDTOList(inboundOrders.get().stream()
                            .map(inboundOrder -> InboundOrderDTO.builder()
                                    .orderId(inboundOrder.getOrderNumber().intValue())
                                    .orderDate(inboundOrder.getOrderDate())
                                    .sectionDTO(SectionDTO.builder()
                                            .sectionCode(inboundOrder.getSection().getId().intValue())
                                            .warehouseCode(inboundOrder.getSection().getWarehouse().getId().intValue())
                                            .build())
                                    .batchDTOList(inboundOrder.getBatchStocks().stream()
                                            .map(batchStock -> BatchDTO.builder()
                                                    .idBatch(batchStock.getId().intValue())
                                                    .idProduct(batchStock.getProduct().getId().intValue())
                                                    .currentTemperature(batchStock.getCurrentTemperature())
                                                    .minimunTemperature(batchStock.getMinimumTemperature())
        ...
        return new InboundListResponseDTO();
    }
