...
public class ProductController {
    ...
    public ResponseEntity<?> getProductLocationById(@PathVariable int idProduct,
                                                    @RequestParam(name = "order", required = false, defaultValue = "L") String order) {
        Authentication authentication = SecurityContextHolder
                .getContext()
                .getAuthentication();
        int managerId = ((UserEntity) authentication.getPrincipal()).getId();
        return new ResponseEntity<>(service.getProductLocationById(idProduct, managerId, order),
                HttpStatus.OK);
    }
    ...
    @GetMapping("/filter")
    public ResponseEntity<List<ProductDetailResponseDto>> searchProductsByFilter(@Valid @RequestBody ProductFilterRequestDTO request) {
        return ResponseEntity.ok(service.getProductsByFilter(request));
    }
