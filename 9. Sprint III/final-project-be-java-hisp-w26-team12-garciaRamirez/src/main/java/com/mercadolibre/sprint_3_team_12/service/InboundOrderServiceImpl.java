...
public class InboundOrderServiceImpl implements IInboundOrderService{
    ...
    private void validateDates(BatchDTO batchDTO, Long idProduct) {
        ...
        LocalDate dueDate = batchDTO.getDueDate();
        if (dueDate.isBefore(LocalDate.now())) {
            throw new ApiException(
                    "Invalid due date",
                    "Due date for product with id " + idProduct + " in batch number " + batchDTO.getIdBatch() + " cannot be in the past",
                    HttpStatus.BAD_REQUEST.value());
        }
        LocalDate manufacturingDate = batchDTO.getManufacturingDate();
        if (manufacturingDate != null) {
            if (manufacturingDate.isAfter(dueDate)) {
                throw new ApiException(
                        "Invalid manufacturing date",
                        "Manufacturing date for product with id " + idProduct + " in batch number " + batchDTO.getIdBatch() + " cannot be after the due date",
                        HttpStatus.BAD_REQUEST.value());
            }
        }
    }
