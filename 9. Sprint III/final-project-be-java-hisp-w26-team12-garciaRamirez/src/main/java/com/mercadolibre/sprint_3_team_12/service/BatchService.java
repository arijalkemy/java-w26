...
import com.mercadolibre.sprint_3_team_12.repository.ISectionRepository;
import com.mercadolibre.sprint_3_team_12.repository.IUserRepository;
...
public class BatchService implements IBatchService {
     ...
     * @throws ApiException - If an exception occurs while retrieving the
     ...
     *
     * @param scarceLimit The threshold of scarcity, which should be a positive number.
     * @return A list of ScarceWarehouseDTO objects representing warehouses and their scarce products.
     * @throws ApiException If the scarceLimit is not positive.
     */
    @Override
    public List<ScarceWarehouseDTO> getAllScarceProducts(Long scarceLimit){

        if(scarceLimit <= 0){
            throw new ApiException("Bad request", "Scarce limit must be positive or zero", HttpStatus.BAD_REQUEST.value());
        }

        //Fetch all scarce products in scarceWarehouseProjection format
        List<ScarceWarehouseProjection> scarceWarehouseProjections = batchStockRepository.findScarceProductsInAllWarehouses(scarceLimit);

        //Creation of map to avoid duplicates and organize better
        Map<Long, ScarceWarehouseDTO> warehouseMap = new HashMap<>();

        // warehouseList structuration
        //Although for structures are not recommended, in this case we needed to iterate over all the items making complex logic, thus the for implementation
        for (ScarceWarehouseProjection projection : scarceWarehouseProjections) {
            Long warehouseID = projection.warehouseID();
            Long productID = projection.productID();
            Long productQuantity = projection.productQuantity();

            //Check for dduplicates
            if (warehouseMap.containsKey(warehouseID)) {
                ProductDTO productDTO = new ProductDTO(productID.intValue(), productQuantity.intValue());
                warehouseMap.get(warehouseID).getProducts().add(productDTO);
            } else {

                List<ProductDTO> products = new ArrayList<>();
                products.add(new ProductDTO(productID.intValue(), productQuantity.intValue()));

                ScarceWarehouseDTO warehouseDTO = new ScarceWarehouseDTO(warehouseID, products);
                warehouseMap.put(warehouseID, warehouseDTO);
            }
        }
