...
public class ProductController {
    ...
    ResponseEntity<ProductCreationResponseDTO> postProducts(@PathVariable Long idSeller, @RequestBody ProductCreationListRequestDTO productCreationDTO){
        return ResponseEntity.ok(productService.postProduct(productCreationDTO, idSeller));
    }

    @PutMapping("/{idSeller}/{idProduct}")
    ResponseEntity<ProductCreationResponseDTO> putProduct(@PathVariable Long idSeller, @PathVariable Long idProduct, @RequestBody ProductCreationDTO requestDTO){
        return ResponseEntity.ok(productService.updateProduct(idProduct, requestDTO, idSeller));
    }
}
