...
public class ControllerExceptionHandler {
 ...
 public ResponseEntity<ApiErrorDetailed> handlerNotFountEntitiesException(ApiExceptionDetailed exception) {  
  return ResponseEntity.status(exception.getStatusCode()).body(
    ...
    protected ResponseEntity<ApiError> handleReturnAlreadyExistsException(ReturnAlreadyExistsException e) {
        LOGGER.error("Return already exists.", e.getMessage());
        NewRelic.noticeError(e);

        ApiError apiError = new ApiError(
                "Return already exists.", e.getMessage(), HttpStatus.CONFLICT.value());
        return ResponseEntity.status(apiError.getStatus()).body(apiError);
    }

    @ExceptionHandler(PurchaseOrderNotFoundException.class)
    protected ResponseEntity<ApiError> handlePurchaseOrderNotFoundException(PurchaseOrderNotFoundException e) {
        LOGGER.error("Purchase order not exists", e.getMessage());
        NewRelic.noticeError(e);

        ApiError apiError = new ApiError(
                "Purchase order not exists", e.getMessage(), HttpStatus.NOT_FOUND.value());
        return ResponseEntity.status(apiError.getStatus()).body(apiError);
    }

    @ExceptionHandler(ReturnNotFoundException.class)
    protected ResponseEntity<ApiError> handleReturnNotFoundException(ReturnNotFoundException e) {
        LOGGER.error("No returns found", e.getMessage());
        NewRelic.noticeError(e);

        ApiError apiError = new ApiError(
                "No returns found", e.getMessage(), HttpStatus.NOT_FOUND.value());
        return ResponseEntity.status(apiError.getStatus()).body(apiError);
    }

    @ExceptionHandler(ErrorUpdatingReturnException.class)
    protected ResponseEntity<ApiError> handleErrorUpdatingReturnException(ErrorUpdatingReturnException e) {
        LOGGER.error("Error updating return", e.getMessage());
        NewRelic.noticeError(e);

        ApiError apiError = new ApiError(
                "Error updating return", e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR.value());
        return ResponseEntity.status(apiError.getStatus()).body(apiError);
    }
}
