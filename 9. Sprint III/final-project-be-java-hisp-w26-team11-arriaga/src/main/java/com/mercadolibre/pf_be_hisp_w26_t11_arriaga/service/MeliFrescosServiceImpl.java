...
import com.mercadolibre.pf_be_hisp_w26_t11_arriaga.exceptions.ApiException;
import com.mercadolibre.pf_be_hisp_w26_t11_arriaga.exceptions.BadRequestException;
import com.mercadolibre.pf_be_hisp_w26_t11_arriaga.exceptions.NotFoundException;
import com.mercadolibre.pf_be_hisp_w26_t11_arriaga.model.*;
import com.mercadolibre.pf_be_hisp_w26_t11_arriaga.repository.*;
import com.mercadolibre.pf_be_hisp_w26_t11_arriaga.util.ValidationUtils;
...
public class MeliFrescosServiceImpl implements IMeliFrescosService {
    ...
    public List<BatchStockInboundOrderResponseDto> getInboudOrder(Integer inboundorderId) {
        List<Batch>  response = batchRepository.findInboundOrderWithBatches(inboundorderId);

        if (response.isEmpty()) {
            throw new ApiException("Inbound Order not found", "inbound_order_not_found", 404);
        }


        return response.stream()
                .map(batch -> modelMapper.map(batch, BatchStockInboundOrderResponseDto.class))
                .toList();
    }

    public RepresentativeResponseDTO getRepresentativeByCodeWarehouse(Integer warehouseId) {
        List<Object[]> response = warehouseRepository.findByRepresentativeOrderByWarehouseCode(warehouseId);
        System.out.println(response);
        if (response.isEmpty()) {
            throw new ApiException("Warehouse not found", "warehouse_not_found", 404);
        }


        return RepresentativeResponseDTO.builder().warehouseCode((Integer) response.get(0)[0])
                .firstName(response.get(0)[1].toString())
                .lastName(response.get(0)[2].toString())
                .role((Role) response.get(0)[3])
                .build();
    }
