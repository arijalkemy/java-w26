...
import com.mercadolibre.pf_be_hisp_w26_t11_arriaga.service.AuthServiceImpl;
...
//                .andExpect(jsonPath("$.batch_stock_extended_dtolist[1].due_date").value(currentDate.plusDays(7).toString()));
    ...
    @Test
    @DisplayName("RE 6: devolver los batch por id de order ok")
    public void getBatchByOrderIdOk() throws Exception {
        // Arrange
        Integer orderId = 1;
        // Act & Assert
        AuthResponseDto authResponseDto = authService.login(new LoginRequestDto("rep_user1", "password1"));
        this.mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/fresh-products/inboundorder/" + orderId )
                        .header("Authorization", "Bearer " + authResponseDto.getToken())
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk());
    ...
    @Test
    @DisplayName("RE 6: devolver los batch por id de order no ok")
    public void getBatchByOrderIdNoOk() throws Exception {
        // Arrange
    ...
    @Test
    @DisplayName("RE 6: regresar representative con warehouse ok")
    public void getRepresentativeByWarehouseOk() throws Exception {
        // Arrange
        Integer warehouseCode = 1001;
        // Act & Assert
        AuthResponseDto authResponseDto = authService.login(new LoginRequestDto("rep_user1", "password1"));
        this.mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/fresh-products/warehouse/" + warehouseCode )
                        .header("Authorization", "Bearer " + authResponseDto.getToken())
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk()).andExpect(MockMvcResultMatchers.jsonPath("$.warehouse_code").value(warehouseCode));
    }

    @Test
    @DisplayName("RE 6: regresar representative con warehouse no ok")
    public void getRepresentativeByWarehouseNoOk() throws Exception {
        // Arrange
        Integer warehouseCode = 1000;
        // Act & Assert
        AuthResponseDto authResponseDto = authService.login(new LoginRequestDto("rep_user1", "password1"));
        this.mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/fresh-products/warehouse/" + warehouseCode )
                        .header("Authorization", "Bearer " + authResponseDto.getToken())
