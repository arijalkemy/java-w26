...
public class InboundOrderServiceImpl implements IInboundOrderService {
    ...
    /**
     * Este método busca en la base de datos todas las ordenes de ingreso, a partir de un id de warehouse,
     * valida que el warehouse exista, que el usuario sea representante del warehouse
     * en caso de que alguna de estas validaciones falle se lanzará una excepción.
     * @param idWarehouse int
     * @param emailUser String
     * @return List<InboundOrderResponseDTO>
     */
    @Override
    public List<InboundOrderResponseDTO> getAllByIdWarehouse(int idWarehouse, String emailUser){
        //Buscar warehouse, tira excepcion si no existe
        Warehouse warehouse = warehouseService.findWarehouseById(idWarehouse);

        userIsManagerWarehouse(warehouse, emailUser);

        List<InboundOrder> inboundOrderList = inboundOrderRepository.findAllBySection_Warehouse_Id(idWarehouse);

        if(inboundOrderList.isEmpty()){
            throw new ApiException("Not Found","No existen ordenes de carga del warehouse con id "+idWarehouse,404);
        }

        return inboundOrderList.stream().map(InboundOrderMapper::inboundOrderToResponseDto).toList();

    }

    @Override
    public InboundOrderResponseDTO getById(int idOrder, String emailUser) {

        Optional<InboundOrder> inboundOrder = inboundOrderRepository.findById(idOrder);

        if(inboundOrder.isEmpty()){
            throw new ApiException("Not Found","No existe una inbound order con el id: "+idOrder,404);
        }

        if(!inboundOrder.get().getSection().getWarehouse().getManager().getEmail().equalsIgnoreCase(emailUser)){
            throw new ApiException("Forbidden",
                    "El usuario autenticado no es representante del warehouse al que pertenece la orden.",
                    403);
        }

        return InboundOrderMapper.inboundOrderToResponseDto(inboundOrder.get());
    }
