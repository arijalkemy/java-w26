...
public class CodigoMorseServiceImpl implements ICodigoMorseService {
    ...
    public String getTraduccionAlfabetoAMorse(String texto) {
        /*
        String[] listaAlfabetoPalabrasInput = texto.split(" ");
        String textoFinal = "";

        for (int i = 0; i < listaAlfabetoPalabrasInput.length; i++){
            String[] listaAlfabetoLetraInput = listaAlfabetoPalabrasInput[i].split(" ");

            for (int j = 0; j < listaAlfabetoLetraInput.length; j++){

                for (Map.Entry<Character, String> entry : mapaMorse.entrySet()) {
                    if (entry.getValue().equals(listaAlfabetoLetraInput[j])) {
                        textoFinal += entry.getKey();
                        break;
                    }
                }
