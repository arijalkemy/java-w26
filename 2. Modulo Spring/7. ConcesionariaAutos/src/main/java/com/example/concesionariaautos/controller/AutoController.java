...
public class AutoController {
    ...
    @GetMapping("/prices")
    public List<Auto> getCarsByPrice(@RequestParam String since, @RequestParam String to) {
        return serviceAuto.getCarsByPrice(since, to);
    }

    @GetMapping("/{id}")
    public List<Auto> getCarsById(@PathVariable String id) {
        return serviceAuto.getCarById(id);
    }
}
