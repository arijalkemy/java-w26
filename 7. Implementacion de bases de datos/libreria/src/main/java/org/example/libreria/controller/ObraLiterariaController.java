...
public class ObraLiterariaController {
    ...
    public ResponseEntity<?> findTopPages(@RequestParam Integer year) {
        return new ResponseEntity<>(obraLiterariaService.getObrasLiterariasBeforYearPublication(year), HttpStatus.OK);
    }

    @GetMapping("/find/editorial")
    public ResponseEntity<?> findByEditorial(@RequestParam String editorial) {
        return new ResponseEntity<>(obraLiterariaService.getObrasLiterariasByEditorial(editorial), HttpStatus.OK);
    }
