...
public class ClothesServiceImpl implements IClothesService {
    ...
    public ClothesResponseDTO getClothesByCode(Long code) {
        Clothes clothes = clothesRepository.findClothesByCode(code);
        if (clothes != null) {
    ...
    public List<ClothesResponseDTO> getClothesBySize(String size) {
        return clothesRepository.findClothesBySize(size).stream().map(this::mapToDTO).toList();
    }

    @Override
    public List<ClothesResponseDTO> getClothesByType(String type) {
        return clothesRepository.findClothesByType(type).stream().map(this::mapToDTO).toList();
    }
