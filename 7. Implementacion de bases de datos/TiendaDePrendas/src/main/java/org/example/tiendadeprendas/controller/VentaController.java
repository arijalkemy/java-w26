...
public class VentaController {
    ...
    @GetMapping()
    public ResponseEntity<?> getPrendaByTimeSell(@RequestParam LocalDate date) {
        return ResponseEntity.status(HttpStatus.OK).body(ventaService.findAllPrendasByFecha(date));
    }

    @GetMapping("/clothes/{number}")
    public ResponseEntity<List<PrendaDto>> getPrendasBySale(@PathVariable Long number){
    ...
    @PutMapping("/{number}")
    public ResponseEntity<?> updateVentaByNumber(@PathVariable Long number, @RequestBody VentaDto ventaDto){
        ventaService.updateVenta(number, ventaDto);
        return ResponseEntity.ok().build();
    }

    @DeleteMapping("/{number}")
    public ResponseEntity<?> deleteVentaByNumber(@PathVariable Long number){
