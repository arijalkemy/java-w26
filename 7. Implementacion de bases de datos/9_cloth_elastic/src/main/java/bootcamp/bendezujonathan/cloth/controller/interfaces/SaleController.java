...
public interface SaleController {
    ...
    @GetMapping("/{id}")
    ResponseEntity<SaleResponse> getById(@PathVariable String id);
    ...
    ResponseEntity<List<ClothResponse>> getAllClothOfSale(@PathVariable String id);

    @PutMapping("/{id}")
    ResponseEntity<Void> putSale(@PathVariable String id, @RequestBody SaleRequest request);

    @DeleteMapping("/{id}")
    ResponseEntity<Void> deleteSale(@PathVariable String  id);
