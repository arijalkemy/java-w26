...
import org.example.showroom.services.IClothesService;
...
public class ClothesController {
    ...
    @GetMapping("/{code}")
    ResponseEntity<ClothesResponseDTO> getClotheByCode(@PathVariable String code){
        return new ResponseEntity<>(clothesService.findByCode(code),HttpStatus.OK);
    }

    @PutMapping("/{code}")
    ResponseEntity<ClothesResponseDTO> putClotheByCode(@PathVariable String code, @RequestBody ClothesRequestDTO clothe){
        return new ResponseEntity<>(clothesService.updateClotheByCode(code, clothe),HttpStatus.OK);
    }

    @DeleteMapping("/{code}")
    ResponseEntity<String> deleteClotheByCode(@PathVariable String code){
