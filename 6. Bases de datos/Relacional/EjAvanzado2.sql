...
-- 1
SELECT dep.nombre_depto, emp.puesto, dep.localidad FROM DEPARTAMENTO dep
INNER JOIN EMPLEADO emp ON emp.depto_nro = dep.depto_nro;

-- 2
SELECT dep.* FROM DEPARTAMENTO dep
INNER JOIN EMPLEADO emp ON emp.depto_nro = dep.depto_nro
...
INNER JOIN DEPARTAMENTO d
ON e.depto_nro = d.depto_nro
WHERE e.salario = (SELECT MIN(salario) FROM EMPLEADO e);
