...
SELECT s.title 'Título', COUNT(*) as 'Cantidad de tempor	adas'
FROM seasons se INNER JOIN series s ON se.serie_id = s.id
GROUP BY se.serie_id;

-- Mostrar el nombre de todos los géneros y la cantidad total de películas por cada uno, siempre que sea mayor o igual a 3.

SELECT g.name, COUNT(*) totalMovies FROM genres g INNER JOIN movies m ON g.id = m.genre_id
GROUP BY g.id HAVING totalMovies > 3;
