...
public class ExceptionController {
    ...
    public ResponseEntity<ExceptionDTO> httpMessageNotReadableException(HttpMessageNotReadableException ex){
        return ResponseEntity.badRequest().body(new ExceptionDTO(ex.getClass().getSimpleName()));
    }

    @ExceptionHandler(HandlerMethodValidationException.class)
    public ResponseEntity<ExceptionDTO> handlerMethodValidationException(HandlerMethodValidationException ex) {
        ExceptionDTO exceptionDTO = new ExceptionDTO(ex.getAllErrors().stream().map(e -> e.getDefaultMessage()).toList().toString(), ex.getBody().getDetail());
