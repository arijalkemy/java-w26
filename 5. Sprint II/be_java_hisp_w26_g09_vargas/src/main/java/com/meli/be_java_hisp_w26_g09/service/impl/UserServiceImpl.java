...
public class UserServiceImpl implements IUserService {
    ...
    public UserDTO getFollowersById(Integer id) {
        ...
        List<User> followers = users.stream()
                .filter(user -> user.getFollowed() != null && user.getFollowed().stream().map(User::getUserId)
                        .anyMatch(userId -> userId.equals(userFollowers.get().getUserId())))
                .toList();
    ...
    public ResponseDTO unfollowUser(int userId, int userIdToUnfollow) {
        ...
        if (userWhoUnfollowOptional.isEmpty())
            throw new NotFoundException("User not found");
        if (userWhoUnfollowOptional.get().getRole() == null
                || userWhoUnfollowOptional.get().getRole().getIdRole().equals(Role.ID_SELLER))
            throw new BadRequestException("The seller can't unfollow to a seller");
        if (userWhoUnfollowOptional.get().getFollowed() == null
                || userWhoUnfollowOptional.get().getFollowed().isEmpty())
    ...
    public ResponseDTO follow(Integer userId, Integer userIdToFollow) {
        ...
        if (!customer.getRole().getIdRole().equals(Role.ID_CUSTOMER)
                || !seller.getRole().getIdRole().equals(Role.ID_SELLER))
