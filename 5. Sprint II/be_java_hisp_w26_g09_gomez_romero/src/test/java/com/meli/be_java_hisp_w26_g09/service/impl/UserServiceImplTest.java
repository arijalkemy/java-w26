...
class UserServiceImplTest {
    ...
    void testGetAllUsers_ReturnUsersList_Success(){
        List<User> users = Arrays.asList(
                new User(1, "JohnDoe", new Role(Role.ID_CUSTOMER, "Customer"), new ArrayList<>()),
                new User(2, "AliceSmith", new Role(Role.ID_SELLER, "Seller"), new ArrayList<>()),
                new User(3, "JaneDoe", new Role(Role.ID_CUSTOMER, "Customer"), new ArrayList<>())
        );
    ...
    @Test
    @DisplayName("Test to get all users, returning an empty list when no users are present")
    void testGetAllUsers_ReturnEmptyList_Success(){
        ...
        assertTrue(expectedUsers.isEmpty() && actualUsers.isEmpty());
    }
    ...
    @Test
    @DisplayName("Test to follow a user not found")
    void testFollowUser_NonExistingUser_ExceptionThrown(){
        when(userRepository.findById(1)).thenReturn(Optional.empty());

        assertThrows(NotFoundException.class, () -> userService.follow(1, 2));
    }
