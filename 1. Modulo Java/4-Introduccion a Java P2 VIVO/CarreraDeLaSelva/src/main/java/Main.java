    ...
    public static void main(String[] args) {
        ...
        for(Inscription inscription: inscriptions){
            String res = String.format("Participant: %s - Age: %d - Category: %s - Amount to Pay: %s - Status: %s",
                    ...
                    (inscription.getAmount() == 0) ? "N/A" : "" + inscription.getAmount(),
        ...
        for(Inscription inscription: inscriptions){
            ...
            if(amountPerCategory.containsKey(key)){
                Double acc = amountPerCategory.get(key) + inscription.getAmount();
                amountPerCategory.put(key, acc);
            }else{
                amountPerCategory.put(key, inscription.getAmount());
            }
        }
