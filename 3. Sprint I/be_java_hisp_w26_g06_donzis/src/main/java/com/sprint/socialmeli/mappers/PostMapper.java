...
public class PostMapper {
    ...
    public static Post mapToEntity(PromoPostRequestDTO promoPostRequestDTO){
        if(!promoPostRequestDTO.getHas_promo()){
            throw new BadRequestException("Invalid promo post request: Promo post created without promo");
        }

        try{
            LocalDate date = LocalDate.parse(promoPostRequestDTO.getDate(), formatter);
            return new Post(
                    mapProductToEntity(promoPostRequestDTO.getProduct()),
                    date,
