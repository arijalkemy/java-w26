...
public class PostMapper {
    ...
    public  List<PostDTO> PostListToPostDTOList(List<Post> posts){
        ...
        posts.forEach(post -> postDtos.add(new PostDTO(post.getUserId(), post.getId(), post.getDate(),
                mapper.convertValue(post.getProduct(), ProductDTO.class), post.getCategory(), post.getPrice(), post.getHasPromo(), post.getDiscount())));
        return  postDtos;
    }
}
