...
public class PostServiceImpl implements IPostService {
    ...
    @Override
    public ResponseDTO addProductInPromoPost(PostDTO postDTO) {
        ...
        if (postDTO.getHasPromo() == null || postDTO.getDiscount() == null)
            throw new BadRequestException("The promo and discount can't be null");
    ...
    @Override
    public SellerPromoDTO countProductsInPromoBySeller(Integer sellerId) {
        Optional<User> seller = userRepository.findById(sellerId);
        if (seller.isEmpty()) throw new NotFoundException("The seller was not found");

        if (seller.get().getRole().getIdRole().equals(Role.ID_CUSTOMER))
            throw new NotFoundException("The user is not a seller");

        int promoCount = (int) postRepository.findAll().stream().filter(post -> post.getUserId().equals(sellerId))
                .filter(post -> post.getHasPromo() == true).count();
        return new SellerPromoDTO(seller.get().getUserId(), seller.get().getUserName(), promoCount, null);
    }

    @Override
    public SellerPromoDTO findProductsInPromoBySeller(Integer sellerId) {
        Optional<User> seller = userRepository.findById(sellerId);
        if (seller.isEmpty()) throw new NotFoundException("The seller was not found");
        if (seller.get().getRole().getIdRole().equals(Role.ID_CUSTOMER))
            throw new NotFoundException("The user is not a seller");
        ...
        List<Post> sellerPosts = postRepository.findAll().stream().filter(post -> post.getUserId().equals(sellerId))
                .filter(post -> post.getHasPromo() == true).toList();

        return new SellerPromoDTO(seller.get().getUserId(), seller.get().getUserName(), null,
                postMapper.PostListToPostDTOList(sellerPosts));
    }
