...
public class PostRepositoryImpl implements PostRepository {
    ...
    @Override
    public Integer getQuantityProductsPromo(Integer sellerId) {
        return postsList.stream().filter(p-> p.getUser().getId()== sellerId &&
                                        p.getHasPromo()).toList().size();
    }

    @Override
    public void modifyPromoPost(ProductsPromoDto productsPromoDto) {
        postsList.stream().filter(p->p.getUser().getId()==productsPromoDto.getUserId())
                .forEach(p-> {p.setHasPromo(true);
                              p.setDiscount(productsPromoDto.getDiscount());});
    }
