...
public class UserRepositoryImpl implements IUserRepository {
    ...
    @Override
    public PostDtoPromoCount getPromoPostCount(User user) {
        PostDtoPromoCount postDtoPromoCount = new PostDtoPromoCount();
        postDtoPromoCount.setPromo_products_count(0);
        for (Post post : user.getPosts()){
            if (post.isHas_promo()){
                postDtoPromoCount.setPromo_products_count(postDtoPromoCount.getPromo_products_count()+1);
            }
        }
        postDtoPromoCount.setUser_name(user.getName());
        postDtoPromoCount.setUserId(user.getId());
        return postDtoPromoCount;
    }

    @Override
    public List<Post> getPromoPostList(User myUser) {

        List<Post> postsPromo = myUser.getPosts().stream()
