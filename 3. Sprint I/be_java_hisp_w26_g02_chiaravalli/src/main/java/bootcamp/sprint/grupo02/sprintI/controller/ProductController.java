...
public class ProductController {
    ...
    public ResponseEntity<MessageResponseDTO> createPromoPost(@RequestBody PromoPostDTO dto){
        return ResponseEntity.status(HttpStatus.OK).body(postService.createPromoPost(dto));

    }

    @GetMapping("/promo-post/count")
    public ResponseEntity<PromoPostResponseDTO> getPromoPosts(@RequestParam int user_id){
