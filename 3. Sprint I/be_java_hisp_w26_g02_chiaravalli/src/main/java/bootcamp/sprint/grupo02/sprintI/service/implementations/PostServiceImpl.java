...
public class PostServiceImpl implements PostService {
    ...
    public PromoPostResponseDTO countPromoPosts(int id) {
        ...
        // Contar los productos en promoción del vendedor
        int promoProductCount = (int) sellerPosts.stream()
                .filter(Post::isHasPromo)
                .count();
        return new PromoPostResponseDTO(seller.getId(), seller.getName(), promoProductCount);
    }
    ...
    private Post parsePostDtoToModelWithOutDiscount(PostDTO dto){
        ...
        Post post = new Post(repository.findAll().size() + 1,dto.getUserId(),dateFormater(dto.getDate()),
                dto.getCategory(),dto.getPrice(), product, 0,false);

        return post;

    }
    private Post parsePostDtoToModelWithDiscount(PromoPostDTO dto){
        ProductDTO productDTO = dto.getProduct();

        Product product = new Product(productDTO.getProductId(),productDTO.getProductName(),productDTO.getType(),
                productDTO.getColor(), productDTO.getNotes(),productDTO.getBrand());

        Post post = new Post(repository.findAll().size() + 1,dto.getUserId(),dateFormater(dto.getDate()),
                dto.getCategory(),dto.getPrice(), product, dto.getDiscount(), true);
