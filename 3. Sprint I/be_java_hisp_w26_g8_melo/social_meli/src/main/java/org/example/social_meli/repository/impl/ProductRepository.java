...
public class ProductRepository implements IProductRepository {
    ...
    public Post savePost(Post post) {
        if(post.getPost_id()==null){
            int maxId= postList.stream()
                    .mapToInt(Post::getPost_id).max().orElse(0);
            post.setPost_id(maxId+1);
        }
        postList.add(post);
        try {
            saveDataBase();
    ...
    public List<Post> findAllPostsByUserId(Integer userId) {
        return postList.stream().filter(post-> post.getUser_id().equals(userId)).toList();
    }

    @Override
    public List<Post> findAllPromoPost() {
        return postList.stream().filter(Post::getHas_promo).toList();
    }
}
