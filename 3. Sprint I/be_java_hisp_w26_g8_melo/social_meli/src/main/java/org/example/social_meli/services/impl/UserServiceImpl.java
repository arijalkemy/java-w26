...
public class UserServiceImpl implements IUserService {
    ...
    public UserResponseDTO unfollowUser(Integer userId, Integer userIdToUnfollow) {
        if(!userRepository.existsClientById(userId) ||
            !userRepository.existsSellerById(userId))
            throw new  NotFoundException("No se encontró cliente o vendedor con ese Id");
        ...
        Boolean isFollower=  followers.removeIf(follower -> follower.getUser_id().equals(userId));
        if(!isFollower){
            throw new NotFoundException("El usuario :"+userId+", no es seguidor del usuario: "+userIdToUnfollow);
        }
