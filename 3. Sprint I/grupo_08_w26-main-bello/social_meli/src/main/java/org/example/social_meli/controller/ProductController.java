...
public class ProductController {
    ...
    public ResponseEntity<?> postPromoP(@RequestBody PostDTO postDTO){
        return new ResponseEntity<>(productService.savePost(postDTO), HttpStatus.OK);
    }
    /*HU_11*/
    @GetMapping("/promo-post/count")
    public ResponseEntity<?> getPromoPCount(@RequestParam(name="user_id") Integer userId){
        return new ResponseEntity<>(productService.getPromoPCount(userId), HttpStatus.OK);
    }
}
