...
public class ProductRepository implements IProductRepository {
    ...
    public Boolean existsPost(Integer postId) {
        return postList.stream().anyMatch(post -> post.getPost_id() != null && post.getPost_id().equals(postId));
    }
    ...
    @Override
    public Integer getPromoPCount(Integer userId){
        return postList.stream().filter(v->v.getUser_id().equals(userId)&& v.isHas_promo()).toList().size();
    }
}
