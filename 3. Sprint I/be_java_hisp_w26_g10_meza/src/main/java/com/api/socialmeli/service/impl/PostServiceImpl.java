...
public class PostServiceImpl implements IPostService {
    ...
    @Override
    public void postPromo(PostWithPromoDto post) {
        ...
        for (Post postInList : postList){
            if (postInList.getUser_id() == userId){
                userFound = true;
                if (postInList.getProduct().getProduct_id().equals(post.getProduct().getProduct_id())){
                    productInList = true;
                    break;
                }
            }
        ...
        if (!userFound) {
            throw new NotFoundException("No existe vendedor con ese id");
        } else if (productInList){
            throw new NotFoundException("ya existe el producto en el vendedor con id " + userId);
        }

        Integer postId = (posts.stream().mapToInt(Post::getPost_id).max().orElse(0) + 1);
