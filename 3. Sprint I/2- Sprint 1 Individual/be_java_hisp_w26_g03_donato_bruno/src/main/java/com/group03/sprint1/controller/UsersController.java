...
import com.group03.sprint1.dto.response.DiscountProductsDTO;
...
public class UsersController {
    ...
    public ResponseEntity<DiscountProductsDTO> getDiscountProductsBy(@PathVariable Integer userId) {
        DiscountProductsDTO discountProductsDTO = usersService.getDiscountProductsBy(userId);
        return new ResponseEntity<DiscountProductsDTO>(discountProductsDTO, HttpStatus.OK);
    }

    @GetMapping("/products/promo-post/list?user_id={userId}")
    public ResponseEntity<PublicationDTO> getAllDiscountProducts(@PathVariable Integer userId) {
        PublicationDTO discountProductsDTO = usersService.getAllDiscountProductsBy(userId);
        return new ResponseEntity<PublicationDTO>(discountProductsDTO, HttpStatus.OK);
    }
