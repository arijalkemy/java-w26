...
public class UsersServiceImpl implements IUsersService {
    ...
    @Override
    public DiscountProductsDTO getDiscountProductsBy(Integer userId) {
        //Busco al seller por su id
        Seller seller = usersRepository.findSellerById(userId);
        // Creo la lista de publicaciones que tenga el objeto seller con el id pasado por parametro
        List<Publication> publicationList = seller.getPublications();
        // itero la lista de publicaciones y la filtro por
        List<Publication> discountedPublications = publicationList.stream()
                .filter(Publication::isHasPromo)
                .collect(Collectors.toList());

        // Inicializo una bariable de tipo entero para guardar el dato "promo_products_count"
        Integer howManyProductsDiscounted = discountedPublications.size();

        // Creo el DTO que voy a usar para el response y lo lleno a mano ya que object mapper no comprende el field discountProductCount del dto
        DiscountProductsDTO discountProductsDTO = new DiscountProductsDTO();
        discountProductsDTO.setUserId(seller.getUserId());
        discountProductsDTO.setUserName(seller.getUserName());
        discountProductsDTO.setDiscountProductCount(howManyProductsDiscounted);
        return discountProductsDTO;
    }

    // Bonus
    @Override
    public PublicationDTO getAllDiscountProductsBy(Integer userId) {
        Seller seller = usersRepository.findSellerById(userId);
        List<Publication> publicationList = seller.getPublications();
        return objectMapper.convertValue(publicationList, PublicationDTO.class);
    }
}
