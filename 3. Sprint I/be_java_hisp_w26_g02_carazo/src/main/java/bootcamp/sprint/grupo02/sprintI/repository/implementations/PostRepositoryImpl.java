...
public class PostRepositoryImpl implements PostRepository {
    ...
    public List<Post> findByPromoPost(int sellerId) {
        return this.findBySellerId(sellerId).stream().filter(p -> p.isHasPromo()).toList();
    }

    public List<Post> findByPromoPostProductByType(int sellerId, String type) {
        return this.findBySellerId(sellerId).stream()
                .filter(p -> p.getProduct().getType().equalsIgnoreCase(type)).toList();
    }
