...
public class PostServiceImpl implements PostService {
    ...
    @Override
    public SellerPromoResponseDTO countPromosBySeller(int sellerId) {
        long countPromos = this.searchPostWithPromoBySeller(sellerId).size();
        Seller sellerOwner = this.sellerService.findById(sellerId);

        return SellerPromoResponseDTO.builder()
                .user(new UserResponseDTO(sellerOwner.getId(), sellerOwner.getName()))
                .followersCount(countPromos)
                .build();
    }

    @Override
    public SellerPromoResponseDTO searchPromosBySeller(int sellerId) {
        List<PostResponseDTO> postsWithPromo = this.searchPostWithPromoBySeller(sellerId)
                .stream()
                .map(this::convertToPostResponseDTO)
                .toList();
        Seller sellerOwner = this.sellerService.findById(sellerId);
        return SellerPromoResponseDTO.builder()
                .user(new UserResponseDTO(sellerOwner.getId(), sellerOwner.getName()))
                .posts(postsWithPromo)
                .build();
    }

    private List<Post> searchPostWithPromoBySeller(int sellerId) {
