...
public class PostServiceImpl implements IPostService {
    ...
    @Override
    public ResponseEntity<?> addPromotionalPost(PostDto postDto) {
        try {
            // Verifica si los datos necesarios están presentes y son válidos
            if (postDto == null || postDto.getProduct() == null || !postDto.isHas_promo() || postDto.getDiscount() <= 0) {
                throw new IllegalArgumentException("Missing or invalid data for promotional post.");
            }
            // Realiza la conversión del DTO a entidad y guarda el post
            Post post = convertDtoToEntity(postDto);
            post.setHas_promo(true);
            post.setDiscount(postDto.getDiscount());
            postRepository.save(post);
            // Retorna el post guardado con status 200 OK
            return ResponseEntity.ok(postDto);
        } catch (IllegalArgumentException e) {
            // Retorna un mensaje de error con status 400 Bad Request en caso de datos inválidos
    ...
    public PromoPostCountDto getPromoPostCount(Integer userId) {
        List<Post> posts = postRepository.findAllByUserIdAndHasPromoTrue(userId);
    ...
    @Override
    public Object quantityPromo(Integer userId) {
