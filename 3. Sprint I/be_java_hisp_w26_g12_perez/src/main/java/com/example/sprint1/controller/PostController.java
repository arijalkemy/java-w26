...
public class PostController {
    ...
    // finished method US0005
        ...
        public ResponseEntity<?> postPromo (@RequestBody PostDto postDto){
            ...
            try {
                // Call service to handle logic of adding a new post
                Object response = postService.postPromo(postDto);
                // Return a ResponseEntity with status 200 Ok
                return new ResponseEntity<>(response, HttpStatus.OK);
            } catch (Exception e) {
                // Return a ResponseEntity with status 400 Bad Request
    ...
    @GetMapping("/postsfilter")
    public ResponseEntity<?> getPostsByFilter(@RequestParam(required = false) Integer category,
                                              @RequestParam(required = false) Integer user_id,
                                              @RequestParam(required = false) Boolean has_promo){
        return new ResponseEntity<>(postService.getPostsFilter(category, user_id, has_promo), HttpStatus.OK);
    }
}
