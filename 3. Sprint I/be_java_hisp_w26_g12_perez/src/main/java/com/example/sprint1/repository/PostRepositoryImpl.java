...
public class PostRepositoryImpl implements IPostRepository {
    ...
    @Override
    public Integer getPostInPromoCountByUserId(Integer userId) {
    ...
    @Override
    public List<Post> getPostInPromoByUserId(Integer userId) {
        return listOfPosts.stream()
                .filter(post -> post.getUser_id().equals(userId)
                        && post.isHas_promo())
                .collect(Collectors.toList());
    ...
    @Override
    public List<Post> getPostsFilter(Integer category, Integer user_id, Boolean has_promo) {
        return listOfPosts.stream()
                .filter(post -> category == null || post.getCategory().equals(category))
                .filter(post -> user_id == null || post.getUser_id().equals(user_id))
                .filter(post -> has_promo == null || post.isHas_promo() == has_promo)
                .collect(Collectors.toList());
    }
}
