...
public class PostRepositoryImpl implements PostRepository {
    ...
    @Override
    public List<Post> findPromosBySellerId(int userId) {
        return postsList.stream().filter(p -> p.getUser().getId().equals(userId) && p.getHasPromo() == true).toList();
    }

    @Override
    public Integer countPostWithPromoBySeller(int userId) {

        return (int) postsList
                .stream()
                .filter(p -> p.getUser().getId().equals(userId) && p.getHasPromo() == true)
                .count();
    }
